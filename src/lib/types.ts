...
export type { WorkspacePermissionRow } from "./workspace-permissions";

export type WorkspaceMemberRole = WorkspaceRoleKey;
export type WorkspaceMemberStatusState = "active" | "deactivated";
...
export type WorkspaceCustomEmoji = {
  ...
  sourceDefaultEmoji?: string | null;
...
export type WorkspaceCustomEmojisPage = {
  items: WorkspaceCustomEmoji[];
  total: number;
  page: number;
  pageSize: number;
  totalPages: number;
...
export type WorkspaceEmojiOneClickSlots = [
  string | null,
  string | null,
  string | null,
];
...
export type WorkspaceMembersRow = {
  ...
  membershipStatus: WorkspaceMemberStatusState;
  ...
  avatar: string | null;
  isAway: boolean;
  /** Legacy socket-level field. Khong dung truc tiep de render presence badge. */
  isOnline?: boolean;
...
export type LaterSummary = {
  overdueCount: number;
  nextOverdueAt: IsoDateString | null;
};
...
export type MessageType = "text" | "system" | "timeline" | "huddle";
...
export type MessagesRow = {
  ...
  huddleSnapshot?: HuddleMessageSnapshot | Record<string, unknown> | null;
...
export type AttachmentsRow = {
  ...
  previewUpdatedAt?: IsoDateString | null;
...
export type User = {
  ...
  name?: string | null;
  role?: string | null;
  membershipStatus?: WorkspaceMemberStatusState | null;
  displayName?: string | null;
  avatar?: string | null;
  /** Profile flag do user tu bat/tat. */
  isAway?: boolean;
  /** Legacy realtime/socket field. UI phai derive online cuoi cung tu socket + isAway. */
  isOnline?: boolean;
  status?: string | null;
  namePronunciation?: string | null;
  phone?: string | null;
...
export type Workspace = WorkspacesRow & {
  role: WorkspaceMemberRole;
  memberCount: number;
  permissions?: WorkspacePermissionRow[];
  customEmojis?: WorkspaceCustomEmoji[];
  emojiOneClickSlots?: WorkspaceEmojiOneClickSlots;
};
...
export type Reaction = {
  ...
  userIds: string[];
  /** Snapshot hiển thị của người đã react, nếu API cung cấp */
  users?: {
    id: string;
    name: string | null;
    displayName?: string | null;
...
export type WorkspaceMessageSearchSort = "relevance" | "newest" | "oldest";

export type WorkspaceMessageSearchLocation =
  | {
      kind: "channel";
      channelId: string;
      channelName: string;
...
export type WorkspaceMessageSearchItem = {
  message: Message;
  rank: number | null;
  excerpt: string;
  location: WorkspaceMessageSearchLocation;
};
...
export type NotificationOverrideMode =
  | "all_messages"
  | "mentions_only"
  | "muted";

export type NotificationOverrideSetting = {
  scope: "channel" | "conversation";
  mode: NotificationOverrideMode;
  targetId: string;
  notifyFor: NotificationPreference;
  muteChannel: boolean;
  mutedUntil: IsoDateString | null;
  override: {
    id: string;
    notifyFor: NotificationPreference | null;
    muteChannel: boolean;
    mutedUntil: IsoDateString | null;
  } | null;
  defaults: {
    notifyFor: NotificationPreference;
    notifyOnHereMention: boolean;
