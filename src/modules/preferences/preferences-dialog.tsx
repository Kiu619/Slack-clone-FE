...
import PreferencesLanguageRegion from "./language-region"

const TABS = [
  { id: "notifications", icon: Bell },
  { id: "appearance", icon: Monitor },
  { id: "messages-media", icon: MessageSquare },
  { id: "language", icon: Globe },
  { id: "accessibility", icon: Accessibility },
  { id: "audio-video", icon: Volume2 },
  { id: "privacy", icon: Shield },
  { id: "advanced", icon: Activity },
]
...
export function PreferencesDialog() {
  ...
  const handleTabChange = (tabId: string) => {
    ...
    setActiveTab(tabId)
    setOverlayTransparent(tabId === "appearance" || tabId === "language")
  }
  ...
  const proceedWithNavigation = () => {
    if (isClosingPending) {
      ...
      setOverlayTransparent(pendingTab === "appearance" || pendingTab === "language")
    }
    resetPendingActions()
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex bg-white dark:bg-[#1A1D21] h-162">
        {/* Sidebar */}
        <div className="min-w-52 flex flex-col pt-4 px-3 border-r border-[#2C2E33]">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar overflow-x-hidden">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="px-8 py-6"
              >
                {activeTab === "appearance" && <PreferencesAppearance />}

                {activeTab === "language" && <PreferencesLanguageRegion />}

                {!["appearance", "language"].includes(activeTab) && (
          ...
          {showConfirmation && (
            <div className="absolute bottom-0 left-0 right-0 bg-[#F8F8F8] dark:bg-[#1A1D21] border-t border-[#313338] p-4 flex items-center justify-between z-30 animate-in slide-in-from-bottom-5 duration-300 shadow-2xl">
              <div className="flex flex-col">
                <Typography
                  text={t("saveChangesQuestion")}
                  className="font-bold dark:text-white text-sm"
                />
                <Typography
                  text="This will change the appearance of your Slack."
